    ...
    <div class="container-fluid">
        @if (session('check_success'))
            <div class="alert alert-success">
                {{ session('check_success') }}
            </div>
        @endif
        @if (session('changeStatus'))
            <div class="alert alert-{{ !$website->monitoring_status ? 'danger' : 'success' }}">
        ...
        <div class="d-flex flex-wrap">
            <div class="col-12 col-lg-6">
                ...
                <div class="d-flex justify-content-center justify-content-lg-start">
                    <a class="btn btn-success btn-sm" href="{{ route('personal.manualcheck', $website->id) }}"
                        role="button">Check</a>
                    <a class="btn btn-warning btn-sm ml-3" href="{{ route('personal.website.activate', $website->id) }}"
                        role="button">{{ !$website->monitoring_status ? 'Activate' : 'Deactivate' }}</a>
                    <a class="btn btn-success btn-sm ml-3" href="{{ route('personal.website.edit', $website->id) }}"
                        role="button">Edit</a>
            ...
            @if ($avgExecTime > 0)
                <div class="col-12 col-lg-6 bg-dark text-white mt-lg-0 mt-5" style="border-radius: 5px">
                    <div>
                    ...
                    <div>
                        <div class="mt-4">
                            ...
                            <span style="color: lightgreen; font-size: 24px">last 24 hours
                                ({{ number_format($avgExecTime, 2) }}ms
                                av.)</span>
                            <p style="font-size: 12px" class="mt-2">Shows the "instant" that the monitor started returning
                                a
                                response in ms (and average for the displayed period is
                                {{ number_format($avgExecTime, 2) }}ms).
                            </p>
                        </div>
                        <div class="mt-3 mb-3">
